...
- **Login / identidad:** terminar la integración en entorno real de **OAuth** (Google, Meta, Microsoft): variables `OAUTH_*` en el backend, URLs de callback correctas en cada consola de proveedor, revisión del flujo `/oauth/callback` y pruebas con usuarios reales (además del login email/contraseña ya operativo).
