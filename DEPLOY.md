...
- **Sesión (PR2/PR4):** el JWT va en cookie `pp_access` (HttpOnly) y anti-CSRF en `pp_csrf` + header `X-CSRF-Token` en POST/PUT/PATCH/DELETE cuando hay cookie de sesión. En **producción** la cookie usa por defecto **`SameSite=None`** y **`Secure`** (necesario si el front y el API están en distintos hosts). Si front y API comparten el mismo sitio y querés `SameSite=Lax`, definí `SESSION_COOKIE_SAMESITE=lax` en el backend.
- **Dominio de cookie (opcional):** si necesitás compartir cookie entre subdominios del mismo dominio raíz, definí `SESSION_COOKIE_DOMAIN=.tu-dominio.com`. Si no se define, la cookie queda atada al host del API.
- **Revocación de sesión (PR4):** los JWT incluyen `tokenVersion`; al cambiar password o modificar estado/rol del usuario se invalida la sesión previa. Asegurate de aplicar la migración nueva de Prisma durante el deploy (`prisma migrate deploy`).
